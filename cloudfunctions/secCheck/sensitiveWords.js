...
const CATEGORIES = [
  ...
  },
  {
    ...
    //   已有的「虐猫」已经把「网络虐猫」「虐猫视频」等一切含它的写法一并拦下，
    //   再加一条是冗余。反过来，往 block 里加「虐猫」的近形词要格外小心 ——
    //   子串匹配会连社团自己发的反虐待宣传一起拦掉（「请大家拒绝虐猫」会被拦，见 tests/wordbank.test.js）。
    //
    // 【两个档位的取舍】block = 当场拒绝发布；review = 照常发布 + 落一条 Review 推管理员人工看。
    //   判断依据只有一条：**这个词有没有正当用法**。有正当用法的一律进 review ——
    //   宁可让管理员多看一眼，也不要拦掉社团自己的内容。
    block: [
      '卖猫', '有偿领养', '品种猫', '繁殖', '配种', '虐猫', '打猫', '摔猫', '毒猫', '扔猫', '活埋', '猫肉', '猫皮', '猫骨',
 ...
 *     已经判定的 block。若把它也放前面，一条同时含「火龙果」和「虐猫」的内容
